...
export default function Info({ className, vaultsData }) {
  ...
  const sum = Object.values(vaultsData).reduce((prevSum, vaultData) => prevSum + Number(vaultData.totalTVL || 0), 0);
  const totalTVL = typeof vaultsData.A.totalTVL === 'number' && truncate(sum);

  return (
    <div className={cl(s.root, className)}>
      <div className={cl(s.info, { [s.mobileOpen]: isMobileOpen })}>
      ...
      <a
        className={s.button}
        href="https://solidity.finance/audits/ImmortalFarm/"
        target="_blank"
        rel="noopener noreferrer"
      >
        <img src="images/smartContractAudit.png" alt="Solidity Finance Logo" />
        <span>Audited by Solidity Finance</span>
      </a>
    </div>
  );
}
